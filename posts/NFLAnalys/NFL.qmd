...
NFL_2022Stuffs_EPA <- merge(NFL_2022Stuffs, NFL2022_epa[, c("passer", "receiver", "epa")],   "passer" = "common_variable", all.x = TRUE)
...
filtered_data <- NFL2022_stuffs_EPA %>% filter(passer %in% selected_passers)
...
```{r}
# Assuming you have a data frame named mean_epa_wide from the previous question

library(dplyr)

# Reshape the data frame to long format for summarization
mean_epa_long <- pivot_longer(mean_epa_wide, cols = c("J.Allen", "P.Mahomes"), names_to = "passer", values_to = "mean_epa")

# Summarize the data
summary_data <- mean_epa_long %>%
